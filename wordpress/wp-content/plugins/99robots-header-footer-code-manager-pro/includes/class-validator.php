...
class HFCM_Validator {
	...
	public function validate() {
		...
		while ( ! $this->end() ) {
			...
			if ( $this->end() ) {
				return array(
					'message' => __( 'Parse error: syntax error, unexpected end of snippet.', '99robots-header-footer-code-manager-pro' ),
					'line'    => $token[2],
				);
			}
				...
				if ( $this->check_duplicate_identifier( $structure_type, $token[1] ) ) {
					switch ( $structure_type ) {
						case T_FUNCTION:
							...
							$message = __( 'Cannot redeclare function %s.', '99robots-header-footer-code-manager-pro' );
							break;
						case T_CLASS:
							/* translators: %s: PHP class name */
							$message = __( 'Cannot redeclare class %s.', '99robots-header-footer-code-manager-pro' );
							break;
						case T_INTERFACE:
							/* translators: %s: PHP interface name */
							$message = __( 'Cannot redeclare interface %s.', '99robots-header-footer-code-manager-pro' );
							break;
						default:
							/* translators: %s: PHP identifier name*/
							$message = __( 'Cannot redeclare %s.', '99robots-header-footer-code-manager-pro' );
					}
