...
function commercekit_single_product_wishlist() {
	...
	if ( $commercekit_wishlist ) {
		...
		if ( $wid ) {
			...
			$wsl_page      = isset( $commercekit_options['wsl_page'] ) ? esc_attr( $commercekit_options['wsl_page'] ) : 0;
			$wsl_page_link = get_permalink( $wsl_page );
			$html          = '<div class="commercekit-wishlist full" data-product-id="' . $pid . '"><a href="' . $wsl_page_link . '" class="commercekit-browse-wishlist" title="' . esc_html__( 'Wishlist', 'commercegurus-commercekit' ) . '"><em class="cg-wishlist"></em><span>' . $wsl_brtext . '</span></a></div>';
		} else {
			$wsl_adtext = isset( $commercekit_options['wsl_adtext'] ) && ! empty( $commercekit_options['wsl_adtext'] ) ? commercekit_get_multilingual_string( stripslashes_deep( $commercekit_options['wsl_adtext'] ) ) : commercekit_get_default_settings( 'wsl_adtext' );
			$html       = '<div class="commercekit-wishlist full" data-product-id="' . $pid . '"><a href="#" role="button" class="commercekit-save-wishlist" data-product-id="' . $pid . '" data-type="page" title="' . esc_html__( 'Wishlist', 'commercegurus-commercekit' ) . '"><em class="cg-wishlist"></em><span>' . $wsl_adtext . '</span></a></div>';
		}
	...
	<table class="commercekit-wishlist-table" width="100%">
		...
		<tbody>
			...
			<tr id="product-<?php echo esc_attr( $row['product_id'] ); ?>">
				<td class="remove">
					<a href="#" class="commercekit-remove-wishlist2" data-product-id="<?php echo esc_html( $row['product_id'] ); ?>" data-wpage="<?php echo esc_html( $wpage ); ?>" aria-label="<?php esc_html_e( 'Wishlist', 'commercegurus-commercekit' ); ?>">×</a>
				</td>
				<td class="image">
...
function commercekit_ajax_wishlist_update() {
	...
	$nonce      = wp_verify_nonce( 'commercekit_nonce', 'commercekit_settings' );
	$wlist_mids = isset( $_POST['wlist_mids'] ) ? explode( ',', sanitize_text_field( wp_unslash( $_POST['wlist_mids'] ) ) ) : array();
	$wlist_fids = isset( $_POST['wlist_fids'] ) ? explode( ',', sanitize_text_field( wp_unslash( $_POST['wlist_fids'] ) ) ) : array();
	...
	$table_list = $wpdb->prefix . 'commercekit_wishlist';
	$table_item = $wpdb->prefix . 'commercekit_wishlist_items';

	$uid = (int) get_current_user_id();
	$key = isset( $_COOKIE['commercekit_wishlist'] ) && ! empty( $_COOKIE['commercekit_wishlist'] ) ? sanitize_text_field( wp_unslash( $_COOKIE['commercekit_wishlist'] ) ) : '';
	$sid = ! empty( $key ) ? (int) $wpdb->get_var( $wpdb->prepare( 'SELECT id FROM ' . $table_list . ' WHERE session_key = %s', $key ) ) : 0; // phpcs:ignore

	if ( count( $wlist_ids ) && ( $uid || $sid ) ) {
		$wlist_txt = implode( ',', $wlist_ids );
		if ( $uid ) {
			$sql  = 'SELECT product_id FROM ' . $table_item . ' WHERE user_id = ' . $uid . ' AND product_id IN (' . $wlist_txt . ')';
			$wids = $wpdb->get_col( $sql ); // phpcs:ignore
		} else {
			$sql  = 'SELECT product_id FROM ' . $table_item . ' WHERE list_id = ' . $sid . ' AND product_id IN (' . $wlist_txt . ')';
			$wids = $wpdb->get_col( $sql ); // phpcs:ignore
		}
		...
		if ( count( $wids ) ) {
			...
			foreach ( $wids as $wid ) {
				if ( in_array( (int) $wid, $wlist_mids, true ) ) {
					$wlist         = array();
					$wlist['id']   = $wid;
					$wlist['cls']  = '.mini';
				...
				if ( in_array( (int) $wid, $wlist_fids, true ) ) {
					$wlist         = array();
					$wlist['id']   = $wid;
					$wlist['cls']  = '.full';
