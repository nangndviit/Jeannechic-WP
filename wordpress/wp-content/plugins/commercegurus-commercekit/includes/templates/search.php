						...
						if ( wc_get_loop_prop( 'total' ) ) {
							foreach ( $products->ids as $product_id ) {
								...
								if ( 'product_variation' === $GLOBALS['post']->post_type ) {
									$_variation = wc_get_product( $product_id );

									$GLOBALS['post']->post_title = $GLOBALS['post']->post_title . ' - ' . wc_get_formatted_variation( $_variation, true, false, true );
								}
...
.tabbed-search-results h1 { margin: 0; font-size: 34px; letter-spacing: -0.5px; }
