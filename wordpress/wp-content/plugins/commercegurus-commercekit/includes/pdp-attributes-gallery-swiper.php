...
$layout_template = '<div style="--swiper-navigation-color: #fff; --swiper-pagination-color: #fff" class="swiper-container cg-main-swiper"><ul class="swiper-wrapper cg-psp-gallery" itemscope itemtype="http://schema.org/ImageGallery">{gallery-images}</ul><div class="swiper-button-next"></div><div class="swiper-button-prev"></div></div><div thumbsSlider="" class="swiper-container cg-thumb-swiper cg-thumbs-' . esc_attr( $pdp_attributes_thumbs ) . ' cg-thumbs-count-{img-count}"><ul class="swiper-wrapper" itemscope itemtype="http://schema.org/ImageGallery">{gallery-thumbs}</ul>' . ( isset( $options['pdp_thumb_arrows'] ) && 1 === (int) $options['pdp_thumb_arrows'] ? '<div class="swiper-button-next swiper-button-disabled"></div><div class="swiper-button-prev swiper-button-disabled"></div>' : '' ) . '</div>';
if ( 'vertical-left' === $cgkit_gallery_layout ) {
	$layout_template = '<div thumbsSlider="" class="swiper-container cg-thumb-swiper cg-thumbs-' . esc_attr( $pdp_attributes_thumbs ) . ' cg-thumbs-count-{img-count}"><ul class="swiper-wrapper" itemscope itemtype="http://schema.org/ImageGallery">{gallery-thumbs}</ul></div><div style="--swiper-navigation-color: #fff; --swiper-pagination-color: #fff" class="swiper-container cg-main-swiper"><ul class="swiper-wrapper cg-psp-gallery" itemscope itemtype="http://schema.org/ImageGallery">{gallery-images}</ul><div class="swiper-button-next"></div><div class="swiper-button-prev"></div></div>';
}
?>
...
<div id="commercegurus-pdp-gallery" class="cg-layout-<?php echo esc_attr( $cgkit_gallery_layout ); ?> cgkit-mb10 cgkit-layout-<?php echo esc_attr( $pdp_attributes_thumbs ); ?> <?php echo esc_attr( $one_slider_css ); ?> <?php echo true === $pdp_attributes_lightbox ? 'cg-lightbox-active' : ''; ?>" data-layout-class="cg-layout-<?php echo esc_attr( $cgkit_gallery_layout ); ?>" data-gallery-slug="default_gallery" data-image-ids="<?php echo isset( $cgkit_image_gallery['default_gallery'] ) ? esc_attr( $cgkit_image_gallery['default_gallery'] ) : ''; ?>" <?php echo true === $is_default_variation ? 'style="visibility: hidden"' : ''; ?>>
				...
				if ( count( $cgkit_gallery ) ) {
					foreach ( $cgkit_gallery as $img_key => $attachment_id ) {
						if ( isset( $cgkit_video_gallery['default_gallery'][ $attachment_id ] ) && ! empty( $cgkit_video_gallery['default_gallery'][ $attachment_id ] ) ) {
							$main_video = 0 === $img_key ? true : false;
							echo commercegurus_get_product_gallery_video_html( $cgkit_video_gallery['default_gallery'][ $attachment_id ], $main_video, $pdp_attributes_autoplay, $attachment_id ); // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
						} else {
							if ( 0 === $img_key ) {
								echo commercegurus_get_main_attributes_gallery_image_html( $attachment_id, true, '', true ); // phpcs:disable WordPress.XSS.EscapeOutput.OutputNotEscaped
							} else {
