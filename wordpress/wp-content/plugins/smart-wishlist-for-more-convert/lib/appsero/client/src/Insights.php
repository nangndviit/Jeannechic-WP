...
namespace WLFMC_Appsero;

/**
 * WLFMC_Appsero Insights
 *
...
class Insights {
	...
	public function __construct( $client, $name = null, $file = null ) {
		...
		if ( is_object( $client ) && is_a( $client, 'WLFMC_Appsero\Client' ) ) {
			$this->client = $client;
		}
	}
	...
	public function admin_notice() {
		...
		$optin_url  = wp_nonce_url( add_query_arg( $this->client->slug . '_tracker_optin', 'true', $this->clean_url() ), '_wpnonce' );
		$optout_url = wp_nonce_url( add_query_arg( $this->client->slug . '_tracker_optout', 'true', $this->clean_url() ), '_wpnonce' );
	...
	public function get_post_count( $post_type ) {
		...
		return (int) $wpdb->get_var( "SELECT count(ID) as count FROM $wpdb->posts WHERE post_type = '$post_type' and post_status = 'publish'" );
	}
