...
function shoptimizer_register_required_plugins() {
	$plugins = array(
		...
		array(
			...
			'source'   => 'https://files.commercegurus.com/commercekit/2.2.6/commercegurus-commercekit.zip',
			'required' => true,
			'version'  => '2.2.6',
		),
		array(
...
// xóa phần tử của woocommerce
add_action('wp_head', 'hide_woocommerce_products_header_css');

function hide_woocommerce_products_header_css() {
    echo '<style>';
    echo 'header.woocommerce-products-header { display: none !important; }';
    echo '</style>';
}


 add_action('wp_footer', 'remove_woocommerce_products_header_js');
 function remove_woocommerce_products_header_js() {
     ?>
...
function playground_scripts_and_styles() {
    wp_enqueue_script( 'fa-icons', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css');
}
