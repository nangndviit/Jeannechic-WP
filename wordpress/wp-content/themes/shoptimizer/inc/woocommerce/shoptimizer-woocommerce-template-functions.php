...
if ( 0 < $rating && ! is_product() ) {
        ...
        $count_html = ' <div class="shoptimizer_ratingCount">(' . $rating_cnt .')</div>';
		...
		<?php if ( true === $shoptimizer_layout_woocommerce_prev_next_display ) { ?>
			<div class="shoptimizer-product-prevnext">
				...
				<?php if ( ! empty( $shoptimizer_prev ) ) : ?>
					...
					<a href="<?php echo esc_url( $shoptimizer_prev->get_permalink() ); ?>" aria-label="<?php echo shoptimizer_safe_html( $shoptimizer_prev->get_title() ); ?>">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
				...
				<?php if ( ! empty( $shoptimizer_next ) ) : ?>
					...
					<a href="<?php echo esc_url( $shoptimizer_next->get_permalink() ); ?>" aria-label="<?php echo shoptimizer_safe_html( $shoptimizer_next->get_title() ); ?>">
						<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
if ( ! function_exists( 'shoptimizer_search_modal' ) ) {
	...
	function shoptimizer_search_modal() {
		...
		$shoptimizer_layout_search_recent = '';
		$shoptimizer_layout_search_recent = shoptimizer_get_option( 'shoptimizer_layout_search_recent' );
	...
	<div class="cg-modal search-main-modal fade" tabindex="-1" role="dialog">
	  <div class="modal-dialog" role="document">
		<div class="modal-content">
		  ...
		  <div class="search-modal-heading"><?php echo shoptimizer_safe_html( $shoptimizer_layout_search_title ); ?></div>

		  <?php if ( true === $shoptimizer_layout_search_recent ) { ?>   			
...
function shoptimizer_mobile_filters() {
	if ( is_active_sidebar( 'sidebar-1' ) ) {
		...
		if ( 'no-woocommerce-sidebar' !== $shoptimizer_layout_woocommerce_sidebar ) {
			...
			echo '<a href="#" class="mobile-filter shoptimizer-mobile-toggle"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
if ( ! function_exists( 'shoptimizer_pdp_ajax_atc_enqueue' ) ) {
	...
	function shoptimizer_pdp_ajax_atc_enqueue() {
		if ( is_product() ) {
			wp_enqueue_script('shoptimizer-ajax-script', get_template_directory_uri() . '/assets/js/single-product-ajax.js', array(), NULL, true );
			wp_localize_script(
...
add_action( 'wp_ajax_kktfwp_shoptimizer_update_mini_cart', 'minicart_shoptimizer_update_mini_cart' );
add_action( 'wp_ajax_nopriv_kktfwp_shoptimizer_update_mini_cart', 'minicart_shoptimizer_update_mini_cart' );
...
/**
* Display subcategories on archive pages.
*
* @since 2.6.6
*/

/*
function shoptimizer_display_subcategories_list() {

	$shoptimizer_layout_woocommerce_display_subcategories = '';
	$shoptimizer_layout_woocommerce_display_subcategories = shoptimizer_get_option( 'shoptimizer_layout_woocommerce_display_subcategories' );

    if ( is_product_category() ) {

    	if ( true === $shoptimizer_layout_woocommerce_display_subcategories ) {
	        ...
	        echo '<ul class="shoptimizer-subcategories">';

	        foreach ( $terms as $term ) {
	            $term_link = get_term_link( $term, $taxonomy );
				echo '<li class="'. $term->slug .'"><a href="'. $term_link .'">';
...
add_action('woocommerce_before_shop_loop', 'shoptimizer_display_subcategories_list', 5 );
*/
